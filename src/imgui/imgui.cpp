...
void ImGui::StartMouseMovingWindow(ImGuiWindow* window)
{
	...
	/////////////////////////////////////////////////////////////
	// v1al changes to make dragging by window header only
	const ImVec2 mousePos = GetMousePos();
	const ImVec2 rootPos = window->RootWindow->Pos;
	const ImVec2 rootSize = window->RootWindow->Size;
	const bool needDrag = 
		mousePos.x >= rootPos.x && mousePos.x <= rootPos.x + rootSize.x &&
		mousePos.y >= rootPos.y && mousePos.y <= rootPos.y + 20;

    if (needDrag && !(window->Flags & ImGuiWindowFlags_NoMove) && !(window->RootWindow->Flags & ImGuiWindowFlags_NoMove))
