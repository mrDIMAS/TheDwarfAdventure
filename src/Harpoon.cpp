...
void Harpoon::OnSerialize(Serializer &sav)
{
	UNUSED(sav);
}
...
Harpoon::Harpoon(const shared_ptr<Location> &location) : Weapon(location, ItemType::Weapon_Harpoon)
{
	mModel = location->GetScene()->CreateSprite();
	mModel->AttachTo(mNode);
	mModel->SetTexture(
		game->GetRenderer()->RequestTexture("data/textures/weapons/harpoon_launcher.png"));
...
bool Harpoon::Shoot(const Vec2f &direction)
{
	...
	if (mShootTimeout <= 0)
	{
		...
		{
			WearOut(0.3f);
			location->AddProjectile(make_shared<Projectile>(location, GetOwner(), ItemType::Projectile_Harpoon,
